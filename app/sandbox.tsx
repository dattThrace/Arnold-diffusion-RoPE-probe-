...
export default function ArnoldDiffusionSandbox() {
    ...
    const runChunk = useCallback(function chunkStep() {
        ...
        // Loop aggressively in an optimal ~12ms time slice. We break out slightly before 16.6ms 
        // to leave breathing room for the browser's painting cycle, keeping layout responsive.
        do {
            result = simRef.current.stepForward(5);
        ...
        } while (!result.done && (performance.now() - startTime) < 12);

        const now = performance.now();
        // Throttle UI React state updates to avoid sluggish rendering cycles (refresh displays at ~5 FPS max)
        if (now - lastUpdateRef.current > 200 || result.done) {
            setPoints([...pointsRef.current]);
            setProgress(result.step / 5000);
            
            // Heatmaps update requires deep copy since array buffer is reused internally in engine
            setHeatmaps({ 
                AhA: new Float64Array(result.AhA), 
                AhB: new Float64Array(result.AhB) 
            });
        ...
        if (!result.done) {
            // Next-tick yielding trick bypassing the standard setTimeout clamp: 
            // In modern browsers, setTimeout(fn, 0) clamps to 4ms after a few nestings.
            // MessageChannel posts to the macrotask queue seamlessly, yielding to the render pass 
            // and returning instantly, un-capping iterations to thousands per second.
            if (!channelRef.current) {
                channelRef.current = new MessageChannel();
            }
            channelRef.current.port1.onmessage = chunkStep;
            channelRef.current.port2.postMessage(null);
        } else {
            setPoints([...pointsRef.current]);
            setProgress(1.0);
            setHeatmaps({ AhA: new Float64Array(result.AhA), AhB: new Float64Array(result.AhB) });
            setIsRunning(false);
            simRef.current = null;
        }
    }, [exportSampleRate]);
    ...
    const startSim = () => {
        ...
        // Ignite our event loop hack
        if (!channelRef.current) channelRef.current = new MessageChannel();
        channelRef.current.port1.onmessage = runChunk;
        channelRef.current.port2.postMessage(null);
    };
